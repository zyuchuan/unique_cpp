...
在上面的例子中，`bind`函数将函数`f`和整数`1, 2, 3`“绑定”在一起，并生成了一个新的对象`bind1`，这是一个`callable object`，调用`bind1()`就相当于调用`f(1, 2, 3)`。
...
`std::bind`还有一个巨牛X的功能：绑定占位符。还是刚才的例子，你可以这样绑定：
...
`_1`、`_2`是定义在标准库中的模板类，它们的作用相当于“占位符”，也就是说你可以先用占位符把位子占着，待调用`bind`对象的时候再用实参去替换那些占位符。这是个很神奇的功能，就好像是“先上车，再买票”。多年来我都很好奇它是怎样实现的，今天我就要一探究竟。

## std::bind源代码剖析

`std::bind`的源代码在文件`functional`中：
...
> `__apply_functor`的第三个参数`__tuple_indices<_Indx...>`并没有用到，似乎是多余的。其实不然，编译器需要`__tuple_indeices<_Indx...>`来推导出一些列的`_Indx`的值。

`__apply_functor`内部又调用了`__invoke`，我们对这个函数不做多的纠缠，只要知道它是调用函数`__f(...)`就好。最关键的是这个神秘的`__mu`，它的作用是解析绑定参数。我们知道参数绑定有两种情况，一是构造函数时绑定参数，二是占位符绑定。`__mu`必须能正确地区分这两种情况。

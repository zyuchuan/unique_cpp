...
这是C++标准库的一个惯例：如果类型``T``有``value semantics``，则它的值就是``static constexpr T::value``，同理如果有``type semantics``，则它的类型就是``T::type``。``integral_constant``同时具有两种``semantics``，所以既有``integral_constant::value``，也有``integral_constant::type``。这不是强制要求，但是是一个惯例。
====
...
更有意义，甚至更繁琐。这里只是举个例子说明它的用法，在后面我们会看到``integral_constant``的各种用法，其中之一便是用来定义布尔型常量。

==== 布尔型常量

同整形常量一样，布尔型常量也有对应的元类型``true_type``和``false_type``：

...
促使模板元编程大行其道的最重要的原因就是效率。如果能把尽可能多的工作都放到编译期，那就意味着运行期工作量的下降，也就是说程序会跑得更快。没有什么能阻止C++程序员对效率的向往，如果把工作都放到编译期，那就意味着运行期工作量的下降，那程序必然跑得快，这是模板元编程得以大行其道的最重要的原因。前面已经说过，

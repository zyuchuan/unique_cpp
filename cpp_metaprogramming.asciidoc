...
* 如果类型``T``有``value semantics``，则它的值就定义为``static constexpr T::value``。

* 如果类型``T``有``type semantics``，则它的类型就定义为是``T::type``。

``integral_constant``同时具有两种``semantics``，所以既有``integral_constant::value``，也有``integral_constant::type``。
